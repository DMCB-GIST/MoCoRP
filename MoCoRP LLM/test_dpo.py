...
@dataclass
class DataArguments:
    sft_model_name_or_path: Optional[str] = field(default=None, metadata={"help": "SFT Model checkpoint for weights initialization."})
...
def main():
    ...
    if not os.path.exists(os.path.join(model_config.model_name_or_path, "adapter_config.json")):
        ...
        dpo_peft_config = PeftConfig.from_pretrained(model_config.model_name_or_path)
        model = AutoModelForCausalLM.from_pretrained(dpo_peft_config.base_model_name_or_path, **model_kwargs)
        model = PeftModel.from_pretrained(model, model_config.model_name_or_path)
        model = model.merge_and_unload()

        sft_peft_config = PeftConfig.from_pretrained(data_config.sft_model_name_or_path)
        model = PeftModel.from_pretrained(model, data_config.sft_model_name_or_path)
        model = model.merge_and_unload()
